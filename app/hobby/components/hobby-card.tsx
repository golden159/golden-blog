'use client';

import classNames from 'classnames';
import {
	AnimatePresence,
	motion,
	useIsPresent,
	useReducedMotion,
} from 'motion/react';
import type { ReactNode } from 'react';
import type { HobbyCategory } from '../types';
import CategoryVisual from './category-visual';

type HobbyCardProps = {
	category: HobbyCategory;
	isOpen: boolean;
	onToggle: () => void;
	children: ReactNode;
};

type HobbyPanelProps = {
	id: string;
	labelledBy: string;
	prefersReducedMotion: boolean;
	duration: number;
	children: ReactNode;
};

function HobbyPanel({
	id,
	labelledBy,
	prefersReducedMotion,
	duration,
	children,
}: HobbyPanelProps) {
	const isPresent = useIsPresent();

	return (
		<motion.div
			id={id}
			role='region'
			aria-labelledby={labelledBy}
			aria-hidden={isPresent ? undefined : true}
			inert={isPresent ? undefined : true}
			initial={{ opacity: 0, y: prefersReducedMotion ? 0 : 12 }}
			animate={{ opacity: 1, y: 0 }}
			exit={{ opacity: 0, y: prefersReducedMotion ? 0 : -8 }}
			transition={{ duration }}
			className='border-t border-gray-200 px-6 py-6 dark:border-gray-700 md:px-8 md:py-8'
		>
			{children}
		</motion.div>
	);
}

export default function HobbyCard({
	category,
	isOpen,
	onToggle,
	children,
}: HobbyCardProps) {
	const prefersReducedMotion = useReducedMotion() === true;
	const triggerId = `hobby-${category.id}-trigger`;
	const panelId = `hobby-${category.id}-panel`;
	const duration = prefersReducedMotion ? 0.15 : 0.3;
	const indicatorDuration = prefersReducedMotion ? 0 : 300;

	return (
		<motion.article
			layout={!prefersReducedMotion}
			transition={{ duration, ease: 'easeOut' }}
			className={classNames(
				'overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-950',
				isOpen ? 'md:col-span-12' : category.compactSpan,
			)}
		>
			<div className='p-6 md:p-8'>
				<span className='text-xs font-semibold tracking-[0.2em] text-primary-600 dark:text-primary-400'>
					{category.index}
				</span>
				<h2 className='mt-2'>
					<button
						id={triggerId}
						type='button'
						aria-controls={panelId}
						aria-expanded={isOpen}
						onClick={onToggle}
						className='flex w-full items-start justify-between gap-6 rounded-sm text-left outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-inset'
					>
						<span className='text-2xl font-semibold md:text-3xl'>
							{category.title}
						</span>
						<span
							aria-hidden='true'
							data-testid={`hobby-${category.id}-indicator`}
							className='mt-1 text-2xl text-primary-500 transition-transform ease-out'
							style={{
								transform: `rotate(${isOpen ? 45 : 0}deg)`,
								transitionDuration: `${indicatorDuration}ms`,
							}}
						>
							+
						</span>
					</button>
				</h2>
				<p className='mt-3 text-sm leading-6 text-gray-600 dark:text-gray-300 md:text-base'>
					{category.summary}
				</p>
				<CategoryVisual id={category.id} />
			</div>

			<AnimatePresence initial={false}>
				{isOpen && (
					<HobbyPanel
						key={panelId}
						id={panelId}
						labelledBy={triggerId}
						prefersReducedMotion={prefersReducedMotion}
						duration={duration}
					>
						{children}
					</HobbyPanel>
				)}
			</AnimatePresence>
		</motion.article>
	);
}
